...
results = maximum $ (horizResults ++ vertResults)

calcProds :: Integral a => [a] -> [a]
calcProds x
  | length x == 4 = (foldl (*) 1 x):[]
  | otherwise = (foldl (*) 1 (take 4 x)):[] ++ calcProds (tail x)

makeDiag :: [[a]] -> [[a]]
makeDiag x
  | (length $ last x) == 20 = map (head . last) [x]:[] ++ (makeDiag (init x ++ (tail $ last x):[]))
  | (length $ last x) == 19 = map (head . last) ([x] ++ [init x]):[] ++ (makeDiag ((init $ init x) ++ (tail $ last $ init x):[] ++ (tail $ last x):[]))
  | (length $ last x) == 18 = map (head . last) ([x] ++ [init x] ++ [mf x]):[]
  | otherwise = x

-- Flesh out multi-function 
mf = foldr (.) id (replicate 2 init)
